...
export function renderIn(c) {
    ...
    <style>
        ...
        .timeouts { margin: 0 auto; width: 70%; display: ${c.timeoutsDisplay}; }
        .timeouts div.timeouts2:nth-child(-n + ${c.timeouts[2]})  { opacity: 1; }
        .timeouts div.timeouts1:nth-child(-n + ${c.timeouts[1]})  { opacity: 1; }
        .timeouts1 { height: 6px; border-radius: ${c.toRadius}px; border: ${c.clrOut}px solid ${c.outColor}; width: 20%; background-color: ${c.color[1]}; display: inline-block; margin: 0 auto; position: relative; opacity: 0.2; }
        .timeouts2 { height: 6px; border-radius: ${c.toRadius}px; border: ${c.clrOut}px solid ${c.outColor}; width: 20%; background-color: ${c.color[2]}; display: inline-block; margin: 0 auto; position: relative; opacity: 0.2; }
        ...
        .bar2-length { width: ${c.barLength[2]}%; background-color: ${c.color[2]}; height: 12px; border-radius: 0 ${c.probRadius}px ${c.probRadius}px 0; border: ${c.clrOut}px solid ${c.outColor}; border-left: 0; transition: all 1s ease-out; }
        .bar1-length { width: ${c.barLength[1]}%; background-color: ${c.color[1]}; height: 12px; border-radius: ${c.probRadius}px 0 0 ${c.probRadius}px; border: ${c.clrOut}px solid ${c.outColor}; border-right: 0; transition: all 1s ease-out; }
        .bar-wrapper { display: ${c.barWrapDisplay}; }
